...
//var webserviceIP="${manageHost}";//webservice的ip地址定义在public_header.httl页面
...
function setAreaSelectedDivFun(flag){
	...
	$(".address-confirm").click(function(){
		...
		saveAreaSelectedAll(selectedArea,this.name);
 ...
 * @param selectedArea
 */
function saveAreaSelectedAll(selectedArea,name){
	
	var txtArea;
	
	if(undefined!=selectedArea&&undefined!=selectedArea.pro){
		
		txtArea=selectedArea.pro.proid;
		
		txtAreaPy=selectedArea.pro.proNamePy;//省拼音
		
		//txtAreaName=selectedArea.pro.proname;;
 		if(undefined!=selectedArea.city&&""!=selectedArea.city){
 			txtArea=selectedArea.city.cityid;
 			selectedArea.city="";
 			cityNamePy=selectedArea.city.citynamePy;
 			selectedArea.city="";
 			//txtAreaName=selectedArea.city.cityname;;
 		}
 		
 		txtAreaPy=txtAreaPy+","+cityNamePy;
 		
 		if(undefined!=selectedArea.town&&""!=selectedArea.town){
 			txtArea=selectedArea.town.townid;
 			selectedArea.town="";
 			txtAreaPy=txtAreaPy+","+selectedArea.town.townnamePy;
 			selectedArea.town="";
 			//txtAreaName=selectedArea.town.townname;
 			//防止选择县后，又返回选择市，必须先把县清空
 			//selectedArea.town=null;
 			
 		}
 		
 		txtArea="{"+txtArea+"}";
 	}
//	alert(txtArea+","+txtAreaPy);
	if("c_add"==name){//新增
		$("#infoForm input[name=area]").val(txtArea);
		$("#infoForm input[name=areaPy]").val(txtAreaPy);
		//判断是否需要获取地区面积
		if($("#infoForm input[name=imageArea]").length>0){
			getShapeArea(txtArea);
		};
	}else {//更新
		$("#updateForm input[name=area]").val(txtArea);
		$("#infoForm input[name=areaPy]").val(txtAreaPy);
	}
}
 ...
 * @returns  
 */
function initCityDiv(proId,proname,proNamePy,event){
 ...
 * @returns  
 */
function initTownDiv(cityId,cityname,citynamePy,event){
 ...
 * @returns  
 */
function getTownSelected(townId,townname,townnamePy,event){
	$(event).parent().addClass("active").siblings().removeClass("active");    
...
function initProvinceDivFromAreaList(){
	...
	if(undefined!=areaList.prolist&&areaList.prolist.length>0){
		...
		for(var i=0;i<areaList.prolist.length;i++){
			...
			prostr+='<li><a onclick=\"initCityDiv(\''+pro.adminId+'\',\''+pro.name+'\',\''+pro.namePY+'\',this)\">'+pro.name+'</a></li>';
		}
...
function initCityDivFromcityList(ciytlist){
	...
	if(undefined!=ciytlist&&ciytlist.length>0){
		for(var i=0;i<ciytlist.length;i++){
			citystr+='<li><a onclick=\"initTownDiv('+ciytlist[i].adminId+',\''+ciytlist[i].name+'\',\''+ciytlist[i].namePY+'\',this)\">'+ciytlist[i].name+'</a></li>';
		}
	}
	$(".city").html(citystr);
	$(".select-city").show();
...
function initTownDivFromtownList(townlist){
	...
	if(undefined!=townlist&&townlist.length>0){
		...
		for(var i=0;i<townlist.length;i++){
			townstr+='<li><a onclick=\"getTownSelected('+townlist[i].adminId+',\''+townlist[i].name+'\',\''+townlist[i].namePY+'\',this)\">'+townlist[i].name+'</a></li>';
		}
	}
	$(".town").html(townstr);
	$(".select-town").show();
...
function getAreaListDivFromWebService(type,proId,cityId){
	...
	var url=webserviceIP+"/dataProduct/getAreaList?fatherId="+fatherId;
...
function getSpecialAreaListFromWebService(type,proId,proname,cityid,cityname){
	...
	$.getJSON(url,function(data){
		if(data.code==1){//目前的特殊地区将其子地区划分为县/直辖区两种不同类型的地区，需要将此两种不同的地区合并到一起
			...
			$.getJSON(webserviceIP+"/dataProduct/getAreaList?fatherId="+adminIds[0],function(arealist1){
				if(arealist1.code==1){
					datalist=arealist1.list;
					r = Math.floor(Math.random() * 9999 + 1);
					$.getJSON(webserviceIP+"/dataProduct/getAreaList?fatherId="+adminIds[1],function(arealist2){
						if(arealist2.code==1){
